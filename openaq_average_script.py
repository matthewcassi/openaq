...
def openaq_averages(file_location, time):
    ...
    # read the file from the given location into a pandas dataframe
    df = pd.read_json(file_location)
    
    # set the column names for the dataframe
    df.columns = ['date','value']
    
    # convert epoch time to date time object in pandas
    df['date'] = pd.to_datetime(df['date'], unit='ms')
    
    # set the index as the date to turn it into a time series object
